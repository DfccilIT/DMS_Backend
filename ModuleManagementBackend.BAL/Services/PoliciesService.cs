...
namespace ModuleManagementBackend.BAL.Services
{
    public class PoliciesService : IPolicyService
    {
                    ...
                    async () =>
                    {
                        ...
                        List<PolicyDto> BuildTree(int parentId)
                        {
                            var policies = allPolicies
                                ...
                                .Where(p => !onlyWhatNew || (p.Children.Any() || p.PolicyItems.Any())) 
                                .ToList();
        ...
        public async Task<ResponseModel> GetAllPoliciesWithItemsAndChildren()
        {
            var response = new ResponseModel();
            try
            {
                ...
                var policies = await context.tblPolices
                                             .Where(p => p.ParentPolicyId == 0 && p.status == 0)
                                             .ToListAsync();
                ...
                {
                    var policyWithItems = await GetPolicyWithItemsAndChildren(policy.pkPolId);
                    policyList.Add(policyWithItems);
                }
        ...
        private async Task<PolicyDto> GetPolicyWithItemsAndChildren(int policyId)
        {
            
            var policy = await context.tblPolices
                                       .Where(p => p.pkPolId == policyId && p.status == 0 )
                                       .FirstOrDefaultAsync();
            ...
            {
                pkPolId = policy.pkPolId,
                PolicyHead = policy.PolicyHead
            };

            
            var policyItems = await context.tblPolicyItems
                                            .Where(pi => pi.fkPolId == policyId && pi.status == 0 && pi.WhatNew==true)
                                            .ToListAsync();

           
            policyDto.PolicyItems = policyItems.Select(pi => new PolicyItemDto
            {
                pkPolItemId = pi.pkPolItemId,
                itemSubject = pi.itemSubject,
                itemContent = pi.itemContent,
                itemDescription = pi.itemDescription,
                itemType = pi.itemType,
                docName = pi.docName,
                fileName = pi.filName,
                WhatNew = pi.WhatNew.Value ,
                Url = $"somebaseurl/{pi.filName}", 
                OrderFactor = pi.OrderFactor,
                OfficeOrderDate = pi.officeOrderDate
            }).ToList();

          
            var childPolicies = await context.tblPolices
                                              .Where(p => p.ParentPolicyId == policyId && p.status == 0)
                                              .ToListAsync();

            
            foreach (var childPolicy in childPolicies)
            {
                var childPolicyDto = await GetPolicyWithItemsAndChildren(childPolicy.pkPolId);
                policyDto.Children.Add(childPolicyDto);
            }
